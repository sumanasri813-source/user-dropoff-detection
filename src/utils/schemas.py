...
class PredictionInputModel(BaseModel):
    ...
    session_duration_avg: float = Field(
        ge=0, le=1000, description="Average session duration in minutes"
    )
    feature_count_used: float = Field(
        ge=0, le=500, description="Number of features used"
    )
    ...
    external_user_id: Optional[str] = Field(
        None, max_length=100, description="External user identifier"
    )
    ...
    def validate_positive(cls, v: float) -> float:
        ...
        if not isinstance(v, (int, float)) or (
            isinstance(v, float) and not (-1e308 < v < 1e308)
        ):
...
class BatchPredictionInputModel(BaseModel):
    ...
    return_errors: bool = Field(
        default=True, description="Include validation errors in response"
    )
...
class PredictionResponse(BaseModel):
    ...
    dropoff_probability: float = Field(
        ge=0, le=1, description="Probability of user dropoff"
    )
    predicted_label: int = Field(
        ge=0, le=1, description="Binary prediction (0=stay, 1=dropoff)"
    )
...
def validate_input(data: Dict, model_class: type[BaseModel]) -> BaseModel:
    ...
    try:
        ...
        raise ValidationError(
            f"Input validation failed: {str(e)}", details={"validation_error": str(e)}
        )
