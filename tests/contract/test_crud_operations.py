...
class CrudContractTests(unittest.TestCase):
    ...
    def test_users_create_success(self) -> None:
        ...
        body = resp.get_json()
        self.assertIsNotNone(body, "Response body is None")
    ...
    def test_users_create_duplicate_external_user_id(self) -> None:
        ...
        resp1 = self.client.post("/users", json=payload)
        self.assertEqual(resp1.status_code, 201)

        resp2 = self.client.post("/users", json=payload)
    ...
    def test_users_list_with_data(self) -> None:
        payload1 = {"external_user_id": "user_001", "email": "user1@example.com"}
        payload2 = {"external_user_id": "user_002", "email": "user2@example.com"}
        self.client.post("/users", json=payload1)
        self.client.post("/users", json=payload2)
    ...
    def test_users_get_success(self) -> None:
        create_resp = self.client.post("/users", json={"external_user_id": "user_001", "email": "user@example.com"})
    ...
    def test_users_update_success(self) -> None:
        ...
        update_payload = {"email": "newemail@example.com", "user_segment": "trial"}
        resp = self.client.put(f"/users/{user_id}", json=update_payload)
    ...
    def test_users_update_not_found(self) -> None:
        ...
        resp = self.client.put("/users/9999", json=payload)
        self.assertEqual(resp.status_code, 404)
...
self.assertEqual(body["count"], 0)
def test_predictions_list_response_structure(self) -> None:
resp = self.client.get("/predictions")
body = resp.get_json()
...
def test_predictions_list_with_filters(self) -> None:
        # Create linked users for cleaner prediction records
...
def _stub_predict_one(_model: Any, _payload: Dict[str, Any], _threshold: float, _risk_levels: Dict[str, float]):
return {
...
def test_users_crud_full_workflow(self) -> None:
        ...
        self.assertEqual(user["external_user_id"], "workflow_user")
...
users_data = list_resp.get_json()
self.assertGreaterEqual(users_data["count"], 1)
