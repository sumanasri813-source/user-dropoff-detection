...
def call_api(
    endpoint: str, method: str = "GET", data: Dict = None, timeout: float = API_TIMEOUT
) -> Tuple[bool, Any, str]:
...
def export_predictions_json(predictions: List[Dict]) -> bytes:
    ...
    export_data = {
        ...
        "summary": {
            ...
            "high_risk": sum(
                1 for p in predictions if p.get("dropoff_probability", 0) > 0.7
            ),
            "medium_risk": sum(
                1 for p in predictions if 0.33 <= p.get("dropoff_probability", 0) <= 0.7
            ),
            "low_risk": sum(
                1 for p in predictions if p.get("dropoff_probability", 0) < 0.33
            ),
        },
    }
...
# ============================================================================
    ...
    st.markdown(
        "Enter user behavioral data to predict drop-off risk and receive business recommendations"
    )
    ...
    if submit_button:
        ...
        if not api_connected:
            st.error(
                "❌ **API Unavailable** — Cannot process prediction. Start the API server: `python -m src.api.app`"
            )
        else:
            ...
            with st.spinner("⏳ Analyzing user behavior patterns..."):
                ...
                success, result, error_msg = call_api(
                ...
                if success:
                    ...
                    recommendation = get_business_recommendation(
                        probability, "high" if predicted_label == 1 else "low"
                    )
                    ...
                    st.dataframe(
                        results_table, use_container_width=True, hide_index=True
                    )
...
# ============================================================================
    ...
    st.markdown(
        "Detailed performance metrics, business impact analysis, and model validation"
    )
    ...
    if eval_metrics:
        ...
        metric_info = [
            ...
            (
                ...
                "Accuracy when predicting drop-off",
            ),
            (
                "Recall",
                f"{metrics.get('recall', 0):.4f}",
                "Percentage of actual drop-offs detected",
            ),
            (
                "F1 Score",
        ...
        for i, (name, value, desc) in enumerate(metric_info):
            with cols[i]:
                ...
                st.markdown(
                    f'<p class="metric-desc">{desc}</p>', unsafe_allow_html=True
                )
        ...
        with col1:
            ...
            st.markdown(
                '<p class="metric-desc">Precision-recall area under curve</p>',
                unsafe_allow_html=True,
            )
        ...
        with col2:
            ...
            st.markdown(
                '<p class="metric-desc">Optimized classification boundary</p>',
                unsafe_allow_html=True,
            )
        ...
        with col1:
            ...
            st.markdown(
                '<p class="metric-desc">Correct retentions</p>', unsafe_allow_html=True
            )

        with col2:
            st.metric("True Positives (TP)", int(business.get("tp", 0)))
            st.markdown(
                '<p class="metric-desc">Correct drop-offs</p>', unsafe_allow_html=True
            )

        with col3:
            st.metric("False Positives (FP)", int(business.get("fp", 0)))
            st.markdown(
                '<p class="metric-desc">False alarms</p>', unsafe_allow_html=True
            )

        with col4:
            st.metric("False Negatives (FN)", int(business.get("fn", 0)))
            st.markdown(
                '<p class="metric-desc">Missed detections</p>', unsafe_allow_html=True
            )
        ...
        st.warning(
            "⚠️ Evaluation metrics not available. Run: `python src/evaluation/evaluate_model.py`"
        )
...
# ============================================================================
    ...
    st.markdown(
        "Deep dive into model behavior, feature importance, and user cohort analysis"
    )
    ...
    with col2:
        ...
        st.markdown("**Insight:**")
        st.info(
            "Recency (days since last activity) is the strongest predictor of user drop-off. Users inactive for extended periods are at high risk."
        )
    ...
    st.markdown(
        "Understanding how the model classifies users based on two key features"
    )
    ...
    fig_boundary.add_hline(
        ...
        line_dash="dash",
        line_color="#EF4444",
        annotation_text="High Risk (90 days)",
    )
    fig_boundary.add_vline(
        x=20,
        line_dash="dash",
        line_color="#22C55E",
        annotation_text="Low Risk (20 logins)",
    )
    ...
    fig_dist = go.Figure()
    fig_dist.add_trace(
        go.Histogram(x=all_probs, nbinsx=50, name="All Users", opacity=0.7)
    )
    fig_dist.add_vline(
        x=0.7,
        line_dash="dash",
        line_color="red",
        annotation_text="Decision Threshold (0.70)",
    )
...
# ============================================================================
    ...
    if st.checkbox("⏱️ Auto-refresh every 30 seconds", value=False):
        ...
        st.markdown(
            "*Note: In production, connect to your monitoring dashboard (Datadog, New Relic, etc.)*"
        )
    ...
    with tab1:
        ...
        if st.button("🔄 Generate Sample Batch", use_container_width=True):
            ...
            with st.spinner("⏳ Processing batch predictions..."):
                success, result, message = call_api(
                    "/predict-batch", method="POST", data={"predictions": sample_users}
                )
                ...
                if success:
                    ...
                    results_df["probability_percent"] = (
                        results_df["dropoff_probability"] * 100
                    ).round(2)
                    results_df["risk_category"] = results_df[
                        ...
                        lambda x: (
                            "🔴 High"
                            if x > 0.7
                            else ("🟡 Medium" if x > 0.33 else "🟢 Low")
                        )
                    )
...
Average Drop-off Probability: {np.mean([p['dropoff_probability'] for p in predictions]):.2%}"""
                    ...
                    high_risk_count = sum(
                        1 for p in predictions if p["dropoff_probability"] > 0.7
                    )
                    medium_risk_count = sum(
                        1
                        for p in predictions
                        if 0.33 <= p["dropoff_probability"] <= 0.7
                    )
                    low_risk_count = sum(
                        1 for p in predictions if p["dropoff_probability"] < 0.33
                    )
                    ...
                    with col2:
                        st.metric(
                            "🟡 Medium Risk",
                            medium_risk_count,
                            f"{medium_risk_count/len(predictions)*100:.1f}%",
                        )
                    with col3:
                        st.metric(
                            "🟢 Low Risk",
                            low_risk_count,
                            f"{low_risk_count/len(predictions)*100:.1f}%",
                        )
                    with col4:
                        avg_prob = np.mean(
                            [p["dropoff_probability"] for p in predictions]
                        )
    ...
    with tab2:
        st.info(
            "📤 Upload a CSV file with columns: days_signup_age, recency_days, frequency_total, session_duration_avg, feature_count_used, device_type, os_type, user_segment, region"
        )
...
# ============================================================================
    ...
    st.markdown(
        "Production-ready ML system for user churn prediction with enterprise-grade implementation"
    )
    ...
    st.markdown(
        '<span class="model-badge">✅ SELECTED: Logistic Regression</span>',
        unsafe_allow_html=True,
    )
    ...
    endpoints_df = pd.DataFrame(
        {
            ...
            "Rate Limit": ["Unlimited", "100/min", "100/min", "Unlimited"],
        }
    )
